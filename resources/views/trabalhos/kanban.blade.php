...
<div class="flex gap-3 overflow-x-auto pb-4 justify-center" id="kanban-container">
    @foreach(\App\Models\Trabalho::ESTADOS as $estadoKey => $estadoLabel)
        <div class="kanban-column flex-shrink-0 w-72 min-w-72 rounded-lg border p-3 {{ $columnColors[$estadoKey] ?? 'bg-slate-50 border-slate-200' }}" data-status="{{ $estadoKey }}">
            ...
            <div class="kanban-dropzone min-h-[120px] space-y-2" ondrop="drop(event)" ondragover="allowDrop(event)">
                @foreach($trabalhosPorEstado[$estadoKey] as $trabalho)
                    ...
                    @endphp
                    <div class="kanban-card group cursor-pointer rounded border border-slate-200 bg-white p-3 shadow-sm transition hover:shadow {{ $cardBorderColors[$estadoKey] ?? '' }}"
                         draggable="true"
