...
<div class="mb-6">
    ...
    <h1 class="mt-2 text-2xl font-bold text-slate-800">Editar distrito</h1>
</div>
<form method="post" action="{{ route('distritos.update', $distrito) }}" class="max-w-md space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
    @csrf
    @method('PUT')
    <x-input label="Nome" name="nome" :value="$distrito->nome" required />
